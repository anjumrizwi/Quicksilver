...
//{
//    [ServiceConfiguration(typeof(IMobileProductService), Lifecycle = ServiceInstanceScope.Singleton)]
...
//        public MobileProductService(IContentLoader contentLoader,
...
//        public IEnumerable<MobileVariant> GetVariations(MobileProduct currentContent)
//        {
...
//            var siblingVariant = siblingVariations.OfType<MobileVariant>().FirstOrDefault(x => x.Code == siblingCode);

//            foreach (var variant in siblingVariations.OfType<MobileVariant>())
//            {
//                if (variant.ScreenSize == size && variant.Code != siblingCode && variant.Color == siblingVariant.Color)
//                {
//                    return variant.Code;
//                }
//            }
...
//            var MobileProducts = products.ToList();
//            foreach (var product in MobileProducts)
//            {
...
//            var variations = _contentLoader.GetItems(variationsToLoad.Select(x => x.Key), _preferredCulture).Cast<MobileVariant>();
...
//                var product = MobileProducts.First(x => x.ContentLink == productContentReference);
...
//            var brand = product is MobileProduct ? ((MobileProduct)product).Brand : string.Empty;
